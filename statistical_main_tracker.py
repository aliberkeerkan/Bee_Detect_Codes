...
class TrackedBee:
    def __init__(self, byte_track_id):
        ...
        # --- İSTATİSTİK VERİLERİ ---
        self.total_frames_seen = 0    # Arının toplam görüldüğü kare sayısı
        self.aruco_detected_frames = 0 # ArUco ile doğrulandığı kare sayısı
    ...
    def update_info(self, bbox, class_id, confidence, detected_aruco_id=None):
        ...
        # Arı bu karede görüldü, sayacı artır
        self.total_frames_seen += 1
        
        if detected_aruco_id is not None:
            self.aruco_id = detected_aruco_id
            self.aruco_detected_frames += 1 # ArUco da okunduysa bu sayacı da artır
...
# ==========================================
# 2. MODÜL: GPU Destekli Deteksiyon ve Takip
# ==========================================
class BeeDetector:
...
# ==========================================
# 3. MODÜL: Headless (Ekransız) Veri Toplayıcı
# ==========================================
class VideoFeed:
    ...
    def check_aruco_in_bbox(self, aruco_corners, aruco_ids, bbox):
        if aruco_corners is None or aruco_ids is None or len(aruco_ids) == 0: return None
        x1, y1, x2, y2 = bbox
        flat_ids = aruco_ids.flatten()
        for i in range(len(flat_ids)):
            corner = aruco_corners[i][0]
            center_x, center_y = int(np.mean(corner[:, 0])), int(np.mean(corner[:, 1]))
    ...
    def run(self):
        ...
        if not cap.isOpened():
            print(f"[HATA] {self.video_source} açılamadı!")
            return

        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        print(f"[SİSTEM] Hızlı Analiz (Headless Mode) Başladı.")
        print(f"[SİSTEM] Video toplam {total_frames} kare. Lütfen bekleyin...")
        ...
        while True:
            ...
            frame_count += 1
            # Kullanıcıya işlemin devam ettiğini göstermek için her 500 karede bir bilgi ver
            if frame_count % 500 == 0:
                print(f"> İşlenen Kare: {frame_count} / {total_frames} (%{(frame_count/total_frames)*100:.1f})")
    ...
    def print_final_statistics(self):
        ...
        for bt_id, bee in self.active_bees.items():
            # False Positive (anlık gürültü) olarak sadece 5 kareden az gözükenleri tabloya yansıtma
...
if __name__ == "__main__":
    VIDEO_DOSYASI = "first_one_hour.mp4"
